...
from backfeed.core.teams import send_teams_notification
...
class SecurityGatewayServer(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/api/commodities':
            ...
            self.handle_news()
        elif self.path == '/api/outlook-inbox':
            self.handle_outlook_inbox()
    ...
    def handle_build_info(self):
        try:
            result = {
                "release": "BF-1.0.0-PROD",
                "engine": "Backfeed Cognitive Automation Core v1.0.0",
    ...
    def handle_scratch_list(self):
        try:
            ...
            cfg = load_config()
            dirs = {
                "project": cfg.get("project_scratch_dir"),
                "oddball": cfg.get("oddball_scratch_dir")
            }
            result = {}
    ...
    def do_POST(self):
        ...
        if self.path == '/send-email':
            ...
            try:
                ...
                cfg = load_config()
                target_email = cfg.get("notification_email", "kdp10891@outlook.com")
...
"""
                ...
                # Send Teams notification headlessly if configured
                send_teams_notification(
                    title=f"[Backfeed Sentinel Alert] Heuristic Shield Action: {scenario_title}",
                    text=f"**Shield Decision**: {decision}\n\n**Defensive Reasoning**:\n{reasoning}",
                    color="EF4444" if decision == "Quarantine" else "10B981"
                )
                ...
                print(f"[Server] Sent threat report email for {scenario_title} to {target_email}")
            except Exception as e:
            ...
            try:
                ...
                from backfeed.core import office
                import pythoncom
                pythoncom.CoInitialize()
                try:
                    office.build_presentation(data)
                finally:
...
ERP Distribution Security Gateway"""
            ...
            # Send Teams notification headlessly if configured
            send_teams_notification(
                title="[Market Intel] Merton Stochastic Pricing Shock Triggered",
                text=f"A discontinuous metal/commodity pricing surge has been registered on the LME/CME. High-volatility path simulation has triggered a step jump:\n\n"
                     f"- **Copper**: +{round((multipliers['Copper'] - 1.0)*100, 2)}%\n"
                     f"- **Aluminum**: +{round((multipliers['Aluminum'] - 1.0)*100, 2)}%\n"
                     f"- **Steel HRC**: +{round((multipliers['Steel HRC'] - 1.0)*100, 2)}%",
                color="FF9F1C"
            )
    ...
    def handle_outlook_inbox(self):
        ...
        demo_emails = [
            ...
            },
            {
                ...
                "senderEmail": "kevin.pollock@backfeedagentics.com",
                "recipient": "estimating@vanguard-contractors-llc.net",
                "date": "June 8, 2026 8:15 AM",
                "subject": "Re: RFQ: Deptford Storage Yard Expansion - Quick Quote Needed",
                "snippet": "Hello Vanguard Team, We have processed your RFQ and compiled the quote for the Deptford storage yard...",
                "body": "Hello Vanguard Team,\n\nWe have processed your RFQ and compiled the quote for the Deptford storage yard expansion project. The pricing details are as follows:\n- 1,200 ft Titan B-Line strut channel 14G: $4.10/ft\n- 800 ft Liberty Copper Cable THHN 4/0 AWG: $6.20/ft\n- 500 pcs Matrix Fittings EMT Compression Connector 1/2\": $1.78/pc\n- 25 pcs GFCI SmartGuard Receptacle 20A White (DEV-GFCI): $14.50/pc\n- 4 spools MC Cable 12/2 w/ Ground 250ft (CBL-MC122): $128.00/spool\n\nTotal Estimated Quote: $13,678.00\nDelivery to Deptford Storage site can be scheduled for tomorrow morning if order is finalized. Account Reference: VG-992.\n\nBest regards,\nKevin Pollock\nBackfeed ERP Assistant",
                "folder": "sent",
                "unread": False,
                "hasAttachment": False,
                "attachmentName": "",
            ...
            },
            {
                ...
                "senderName": "Kevin Pollock",
                "senderEmail": "kevin.pollock@backfeedagentics.com",
                "recipient": "kdp10891@outlook.com",
                "date": "June 8, 2026 9:00 AM",
                "subject": "Re: Periodic status update - Agent Status Response",
                "snippet": "Hello, This is your automated status report from Backfeed Copilot. Systems are operating normally...",
                "body": "Hello,\n\nThis is your automated status report from Backfeed Copilot. Systems are operating normally:\n- Docker container status: UP (get-backfeed running on port 8085)\n- Monitored Email: kevin.pollock@backfeedagentics.com\n- Notification Recipient: kdp10891@outlook.com\n- License Key: Verified & Headless Active\n\nLet me know if you would like me to check the status, list the inventory, or trigger a Docker build.\n\nSent via Backfeed Scheduler & Responder Core",
                "folder": "sent",
                "unread": False,
                "hasAttachment": False,
                "attachmentName": "",
        ...
        try:
            ...
            sent_folder = None
            cfg = load_config()
            target_email = cfg.get("outlook_email", "kevin.pollock@backfeedagentics.com")
            for store in namespace.Stores:
                if target_email.lower() in store.DisplayName.lower():
                    ...
                    try:
                        sent_folder = store.GetDefaultFolder(5)  # 5 = Sent Folder
                        print(f"[Server] Synced target store sent items for {store.DisplayName}")
                    except Exception as ex:
                        print(f"[Server] Failed to get sent folder for store {store.DisplayName}: {ex}")
                    break
            if not inbox:
            ...
            if sent_folder:
                try:
                    sent_messages = sent_folder.Items
                    sent_messages.Sort("[ReceivedTime]", True)
                    sent_count = min(15, len(sent_messages))
                    for j in range(1, sent_count + 1):
                        try:
                            msg = sent_messages.Item(j)
                            recipient_name = msg.To
                            recipient_email = ""
                            try:
                                if msg.Recipients.Count > 0:
                                    recipient_email = msg.Recipients.Item(1).Address
                            except Exception:
                                pass
                            ...
                            email_list.append({
                                "id": f"outlook-sent-{j}-{received_time.timestamp()}",
                                "senderName": "Kevin Pollock",
                            ...
                            print(f"Error parsing sent mail item {j}: {sent_err}")
                            continue
                except Exception as sent_folder_err:
                    print(f"Error listing sent folder items: {sent_folder_err}")
...
def start_server(port=PORT, host=''):
    import socketserver
    ...
    print(f"Threaded Server running on {host or 'all interfaces'}:{port}... serving files and handling email dispatches.")
    try:
        httpd.serve_forever()
