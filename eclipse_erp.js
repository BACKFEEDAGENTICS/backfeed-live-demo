/* ============================================================
   Backfeed ERP — Application Logic
   ERP simulation for electrical distribution
   ============================================================ */
...
(function () {
  ...
  function handleAction(action) {
    switch (action) {
      ...
      case 'print':
        ...
        break;
      case 'exit':
        showModal('Exit ERP', '<i class="fa-solid fa-right-from-bracket modal-icon-large warning"></i>Are you sure you want to exit the ERP system? Unsaved changes will be lost.', [
          { label: 'Yes', className: 'modal-btn danger', action: () => { closeModal(); document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#1e1e2e;color:#888;font-family:Segoe UI;font-size:16px;">ERP session ended. Refresh to restart.</div>'; } },
          { label: 'No', className: 'modal-btn', action: closeModal }
        ], 'warning');
        break;
      case 'help-contents':
        showModal('ERP Help', '<i class="fa-solid fa-circle-question modal-icon-large info"></i><b>Backfeed ERP v1.0</b><br>Order Entry System for Electrical Distribution<br><br>For support, contact your Backfeed administrator.<br><br><small>Keyboard: F2=New Order, F3=Search, F5=Refresh, F7=Save, F10=Close Tab</small>', [{ label: 'OK', className: 'modal-btn primary', action: closeModal }], 'info');
        break;
      case 'about':
        showModal('About ERP System', '<div style="text-align:center"><i class="fa-solid fa-bolt" style="font-size:32px;color:#ffd700;margin-bottom:8px;display:block;"></i><b>Backfeed ERP</b><br>Version 1.0.0<br><br>© 2026 Backfeed Agentics<br>Licensed to: Client Company Solutions<br><br><small>Terminal T4 | Mode: Demo</small></div>', [{ label: 'OK', className: 'modal-btn primary', action: closeModal }], 'info');
        break;
      default:
  ...
  // Export for direct invocation from parent app (inlined, no iframe)
  window.startVisualOrderEntry = startVisualOrderEntry;
